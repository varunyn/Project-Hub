...
let createTaskWorkflow: typeof import("../app/lib/taskWorkflow").createTask;
let updateTaskWorkflow: typeof import("../app/lib/taskWorkflow").updateTaskWorkflow;
let deleteTaskWorkflow: typeof import("../app/lib/taskWorkflow").deleteTaskWorkflow;
let retryGithubStatusWorkflow: typeof import("../app/lib/taskWorkflow").retryGithubStatusWorkflow;
let resolveGithubLinkWorkflow: typeof import("../app/lib/taskWorkflow").resolveGithubLinkWorkflow;
...
try {
  ...
  ({ addProject, deleteProject, getProjects, updateProject, resolveProjectPathForServer } =
    await import("../app/utils/projectUtils.js"));
  ({ getAllTasks, getProjectTasks, getTasks, TASK_PRIORITIES, TASK_STATUSES } = await import(
    "../app/utils/taskUtils.js"
  ));
  ({
    createTask: createTaskWorkflow,
    updateTaskWorkflow,
    deleteTaskWorkflow,
    retryGithubStatusWorkflow,
    resolveGithubLinkWorkflow,
  } = await import("../app/lib/taskWorkflow.js"));
} finally {
...
registerTypedTool(
  ...
  async ({ package: packageQuery = "", ecosystem = "", project: projectQuery = "", risk = "" }) => {
    ...
    const updates = report.projects.flatMap((dependencyProject) => {
      ...
      const projectMetadataMatches =
        trackedProject &&
        [trackedProject.id, trackedProject.name].some((value) =>
      ...
      return dependencyProject.updates
        ...
        .map((update) => ({
          project_path: dependencyProject.path,
...
registerTypedTool(
  "create_task",
  {
    ...
    inputSchema: {
      ...
      assignee_id: z.string().max(100).default(""),
...
registerTypedTool(
  ...
  async ({
    ...
    try {
      const { task: updated } = await updateTaskWorkflow({
      ...
      if (
        ...
        "code" in error &&
        (error.code === "task_not_found" || error.code === "project_not_found")
      )
        return result(null);
      throw error;
    }
...
registerTypedTool(
  ...
  async ({ task_id }) => {
    ...
    try {
      await deleteTaskWorkflow(task.projectId, task_id);
      return result(true);
    } catch (error) {
      if (
        error instanceof Error &&
...
registerTypedTool(
  ...
  async ({ project_id, task_id }) => {
    try {
      const outcome = await retryGithubStatusWorkflow({ projectId: project_id, taskId: task_id });
      return result({
        task: outcome.task,
        github_synchronization: outcome.githubSynchronization,
      });
    } catch (error) {
      if (
        error instanceof Error &&
        "code" in error &&
        ["project_not_found", "task_not_found", "task_not_linked"].includes(String(error.code))
      ) {
        return result(null);
      }
      throw error;
...
registerTypedTool(
  ...
  {
    description:
      "Resolve an uncertain GitHub issue link by attaching a caller-verified issue or confirming no issue was created.",
    inputSchema: {
      project_id: z.string(),
      task_id: z.string(),
      resolution: z.discriminatedUnion("action", [
        ...
        z.object({ action: z.literal("confirm-none") }),
      ]),
    },
  },
  async ({ project_id, task_id, resolution }) => {
    try {
      const workflowResolution =
        resolution.action === "attach"
          ? {
              action: "attach" as const,
              issueNumber: resolution.issue_number,
              issueUrl: resolution.issue_url,
            }
          : { action: "confirm-none" as const };
      const outcome = await resolveGithubLinkWorkflow(project_id, task_id, workflowResolution);
      return result(outcome);
    } catch (error) {
      if (error instanceof Error && "code" in error) {
        const code = String(error.code);
        if (["project_not_found", "task_not_found"].includes(code)) return result(null);
        if (["invalid_task", "github_link_not_uncertain", "github_link_conflict"].includes(code))
          return result({ error: error.message, code });
      }
      throw error;
    }
  }
...
registerTypedTool(
  "add_project",
  {
    ...
    inputSchema: {
      ...
      status: projectStatusSchema.default("in progress"),
      readme_preview: z.string().default(""),
...
registerTypedTool(
  "update_project",
  {
    ...
    inputSchema: {
      ...
      readme_preview: z.string().optional(),
...
server.registerResource(
  ...
  { description: "Read-only snapshot of all tracked projects." },
  async (uri) => ({ contents: [{ uri: uri.href, text: JSON.stringify(getProjects()) }] })
);
server.registerResource(
  "tasks",
  "project-hub://tasks",
  { description: "Read-only snapshot of all tasks." },
  async (uri) => ({ contents: [{ uri: uri.href, text: JSON.stringify(getAllTasks()) }] })
);
server.registerResource(
  "project-tasks",
  new ResourceTemplate("project-hub://projects/{project_id}/tasks", { list: undefined }),
  { description: "Read-only tasks for one project." },
  async (uri, variables) => ({
    contents: [
      { uri: uri.href, text: JSON.stringify(getProjectTasks(String(variables.project_id))) },
    ],
  })
);
server.registerResource(
  "dependency-report",
  "project-hub://dependency-report/latest",
  { description: "Latest dependency tracker report." },
  async (uri) => ({ contents: [{ uri: uri.href, text: JSON.stringify(await dependencyReport()) }] })
);
server.registerResource(
  "project-dependency-updates",
  new ResourceTemplate("project-hub://projects/{project_id_or_path}/dependency-updates", {
    list: undefined,
  }),
  { description: "Dependency details for one project." },
  async (uri, variables) => ({
    contents: [
      {
        uri: uri.href,
        text: JSON.stringify(await projectDependencyUpdates(String(variables.project_id_or_path))),
      },
    ],
  })
);
...
async function projectDependencyUpdates(idOrPath: string) {
  ...
  const report = await dependencyReport();
  const dependencyProject = report.projects.find((item) =>
    matchesDependencyProject(item, project, idOrPath)
  );
  return {
    status: report.status === "missing" ? "missing" : dependencyProject ? "ready" : "not_found",
    project: project ?? null,
...
registerTypedPrompt(
  ...
  async ({ package_name, project_id_or_path }) => ({
    messages: [
      {
        role: "user",
        content: {
